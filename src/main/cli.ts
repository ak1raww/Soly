...
const options = {
    ...
    repair: {
        type: "boolean",
        short: "r",
        description: "Re-download Solarcord and restart"
    }
...
const extraOptions = {
    ...
    "ozone-platform": {
        hidden: process.platform !== "linux",
        type: "string",
        description: "Whether to run Soly in Wayland or X11 (XWayland)",
        options: ["x11", "wayland"]
    }
} satisfies Record<string, Option>;
...
export async function checkCommandLineForRepair() {
    ...
    if (State.store.equicordDir) {
        console.error("Cannot repair: using custom Solarcord directory. Remove it in settings first.");
        process.exit(1);
    }

    console.log("Repairing Solarcord...");
...
function checkCommandLineForToggleCommands() {
    ...
    console.error("Soly is not running. Toggle commands require a running instance.");
    app.exit(1);
}
...
function checkCommandLineForQueryCommands() {
    ...
    console.error("Soly is not running. Query commands require a running instance.");
    app.exit(1);
}
...
function checkForSecondInstance() {
    ...
    if (!app.requestSingleInstanceLock({ IS_DEV })) {
        if (!IS_DEV) {
            console.log("Soly is already running. Quitting...");
            app.exit(0);
        }

        console.log("Soly is already running. Quitting previous instance...");
    }
